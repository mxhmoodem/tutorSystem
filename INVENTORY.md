...
- **Sarah Clarke (teacher):** `t1` / `u_sarah` / `s.clarke@centre.co.uk` (roster) vs `t_clarke` / `s.clarke@centre.co.uk` (studentData enrolments, teacherMetrics principal). Two different teacher-ids for the same person.
