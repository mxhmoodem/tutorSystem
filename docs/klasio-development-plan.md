...
| Class identity | Homework, lesson plans and trackers join classes by a free-text **group label** ("Year 10 – Group A"), which is not a column on anything. Renaming a class orphans all three | Everything joins `classes.id`; the label is display text with no join meaning |
