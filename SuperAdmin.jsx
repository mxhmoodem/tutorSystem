...
// Mock data (SA_CENTRES, SA_USERS, SA_REVENUE_TREND, SA_USER_GROWTH,
// SA_ACTIVITY, SA_FEATURE_USAGE, SA_GEOGRAPHIC, SA_DEVICE, SA_TICKETS,
// SA_AUDIT, SA_FLAGS) lives in mocks/superAdmin.mock.jsx, loaded before
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 10, marginBottom: 20 }}>
        {stats.map(s => (
          <div key={s.label} style={{
            background: DS.bg, border: `1px solid ${DS.cardBorder}`, borderRadius: 10, padding: '14px 14px',
    ...
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{
            background: DS.bg, border: `1px solid ${DS.cardBorder}`, borderRadius: 10, padding: '16px 18px',
          }}>
...
// `embedded` is set when rendered as the "Support" section inside the shared
// Communications module (CommunicationsPage) — it drops the outer padding +
// duplicate page header (the host page provides those) so only the
// support-tickets / email-stats content shows.
...
const SuperAdminPages = ({ page }) => {
  switch (page) {
    ...
    // Communications is normally intercepted in index.html for all roles and
    // routed to the shared CommunicationsPage (platform-scoped for superadmin),
    // which embeds SACommsPage as its "Support" section. This case is a safety
    // net if that interception is ever bypassed.
...
Object.assign(window, { SuperAdminDashboard, SuperAdminPages, SACommsPage });
